...
def nonCorrelees(n, nbObjectives, maxValue):
	...
	u = np.ceil(np.random.rand(nbObjectives, n) * maxValue).astype(np.int64)
...
def peuCorrelees(n, nbObjectives, maxValue, maxDiff=None):
	...
	r = (np.random.rand(nbObjectives, n) * (maxDiff * 2) - maxDiff)

	u = np.ceil(p + r).astype(np.int64)
...
def tresCorrelees(n, nbObjectives, maxValue, factor=0.1):
	...
	r = ((np.random.rand(nbObjectives, n) * 2 - 1) * factor) * p
